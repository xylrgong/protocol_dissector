    ...
    def construct(self):
        self.trans = [
            ...
            (s('WAIT_FOR_DISCONNECT') >> s('WAIT_FOR_9_SEC')) + cond(lambda:not self._is_starting) + action(
                self._wait_9),
            # 第二次21252序列
            (s('WAIT_FOR_8_SEC') >> s('WAIT_FOR_AP2_65283')) + cond(self.get_conn('cotp_conn_stt')) + action(self.send_dwnr, dwnr='21252'),
            (s('WAIT_FOR_9_SEC') >> s('WAIT_FOR_AP2_65283')) + cond(self.get_conn('cotp_conn_stp')) + action(self.send_dwnr, dwnr='21252'),
            ...
            (s('WAIT_FOR_AP2_4611_') >> s('WAIT_FOR_AP2_4099')) + cond(self.get_cond(4611, 'ap2_is_4611_')) + action(self.send_dwnr, dwnr='5124'),
            ...
            (s('WAIT_FOR_DISCONNECT_2') >> s('END', final=1)) + cond(lambda:not self._is_starting) + action(
                self._cotp_disconnect),
            # 开启AP的额外过程
            (s('WAIT_FOR_DISCONNECT_2') >> s('WAIT_FOR_94_SEC')) + cond(lambda: self._is_starting, prio=1) + action(
                self._wait_94),
            (s('WAIT_FOR_94SEC') >> s('WAIT_FOR_AP3_65283')) + cond(self.get_conn('cotp_conn3')) + action(self.send_dwnr, dwnr='21252'),
            ...
            (s('WAIT_FOR_AP3_4611_') >> s('WAIT_FOR_AP3_4099')) + cond(self.get_cond(4611, 'ap3_is_4611_')) + action(self.send_dwnr, dwnr='5124'),
            ...
            (s('WAIT_FOR_AP3_4611__') >> s('END', final=1)) + cond(self.get_cond(4611, 'ap3_is_4611__')) + action(self._cotp_disconnect)
        ]

    def parse_args(self, **kwargs):
        self.cotp_skt = kwargs.pop('cotp_skt', None)
        self._is_starting = kwargs.pop('_is_running', False)
